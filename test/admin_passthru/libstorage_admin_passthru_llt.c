...
struct admin_cmd_key_params g_param_arr[] = {
    {g_pci, true, SPDK_NVME_OPC_IDENTIFY, SPDK_NVME_IDENTIFY_CTRLR, true, 0, true},
    ...
    {g_pci, true, SPDK_NVME_OPC_DELETE_IO_SQ, 0, true, 0, false},
...
static void test_get_rpc_from_uio(void)
{
    int wstatus;
    ...
    int fd = libstorage_open("nvme0n1");
    if (fd <= 0) {
        CU_ASSERT(false);
...
int main(int argc, char **argv)
{
    ...
    suite = CU_add_suite("libstorage_rpc", NULL, NULL);
    if (suite == NULL) {
        goto ERROR;
    }

    if (CU_ADD_TEST(suite, test_get_rpc_from_uio) == NULL) {
        goto ERROR;
    }
